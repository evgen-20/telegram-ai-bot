...
async def handle_text(
    ...
    async def on_text_only(text: str, source_msg: Message) -> None:
        ...
        # Re-resolve the backend after any reply-driven engine switch +
        ...
        if current_backend.is_active(key) and target_session_id:
            assert reply_ref is not None
            current_sid = current_backend.get_session_id(key)
            if current_sid != target_session_id:
                ...
                ok = await current_backend.switch_session(key, target_session_id, session_manager)
                if not ok:
                    await source_msg.answer(t("ui.tui_session_missing"))
                    return
                tmux_switched = True
